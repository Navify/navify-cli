(async () => {
	...
	try {
		overrideList = JSON.parse(localStorage.getItem("navify-exp-features"));
	...
	Navify.expFeatureOverride = (feature) => {
		hooksPatched = true;
		...
		localStorage.setItem("navify-exp-features", JSON.stringify(overrideList));
		return feature;
	};

	const content = document.createElement("div");
	content.classList.add("navify-exp-features");
	const style = document.createElement("style");
	style.innerHTML = `
.navify-exp-features .col {
    padding: 0;
}
.navify-exp-features .setting-row::after {
    content: "";
    display: table;
    clear: both;
}
.navify-exp-features .setting-row {
    display: flex;
    padding: 10px 0;
    align-items: center;
}
.navify-exp-features .setting-row .col.description {
    float: left;
    padding-right: 15px;
    width: 100%;
}
.navify-exp-features .setting-row .col.action {
    float: right;
    text-align: right;
}
.navify-exp-features .setting-row .col.action .dropdown {
	width: max-content;
}
.navify-exp-features button.switch {
    align-items: center;
    border: 0px;
    border-radius: 50%;
...
.navify-exp-features button.switch.disabled,
.navify-exp-features button.switch[disabled] {
    color: rgba(var(--spice-rgb-text), .3);
}
.navify-exp-features button.reset {
	font-weight: 700;
	font-size: medium;
...
.navify-exp-features button.reset:hover {
	transform: scale(1.04);
	border-color: var(--spice-text);
}
.navify-exp-features .search-container {
    width: 100%;
}
.navify-exp-features .setting-row#search .col.action {
    position: relative;
    width: 100%;
}
.navify-exp-features .setting-row#search svg {
    position: absolute;
    margin: 12px;
}
.navify-exp-features input.search {
    border-style: solid;
	...
	(function waitForRemoteConfigResolver() {
		...
		if (!hooksPatched || (!Navify.RemoteConfigResolver && !Navify.Platform?.RemoteConfigDebugAPI && !Navify.Platform?.RemoteConfiguration)) {
			setTimeout(waitForRemoteConfigResolver, 500);
			return;
		}

		let remoteConfiguration = Navify.RemoteConfigResolver?.value.remoteConfiguration || Navify.Platform?.RemoteConfiguration;
		const setOverrides = async (overrides) => {
			const remoteConfigDebugAPI = Navify.Platform?.RemoteConfigDebugAPI;
			...
			} else if (Navify.RemoteConfigResolver?.value?.setOverrides) {
				Navify.RemoteConfigResolver.value.setOverrides(Navify.createInternalMap?.(overrides));
			}
		};

		(async function waitForResolver() {
			if (!Navify.RemoteConfigResolver && !Navify.Platform?.RemoteConfigDebugAPI) {
				isFallback = true;
			...
			notice.remove();
			remoteConfiguration =
				Navify?.RemoteConfigResolver?.value.remoteConfiguration ?? (await Navify.Platform?.RemoteConfigDebugAPI.getProperties());
		})();

		for (const key of Object.keys(overrideList)) {
			if (newFeatures.includes(key)) continue;
			delete overrideList[key];
			console.warn(`[navify-exp-features] Removed ${key} from override list`);
			localStorage.setItem("navify-exp-features", JSON.stringify(overrideList));
		}
...
<input type="text" class="search" placeholder="Search for a feature">
		...
		resetBtn.onclick = () => {
			localStorage.removeItem("navify-exp-features");
