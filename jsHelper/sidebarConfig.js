(function SidebarConfig() {
	...
	function writeStorage() {
		...
		return localStorage.setItem("navify-sidebar-config:ylx", JSON.stringify(array));
	}

	const container = document.createElement("div");
	container.id = "navify-sidebar-config";
...
#navify-sidebar-config button:disabled {
...
`;
	...
	function injectInteraction() {
		...
		YLXSidebarState = Navify.Platform.LocalStorageAPI.getItem("ylx-sidebar-state");
	...
	function removeInteraction() {
		...
		if (YLXSidebarState === 1) document.querySelector(".main-yourLibraryX-collapseButton > button")?.click();
			...
			document.documentElement.style.setProperty(
				"--nav-bar-width",
				`${Navify.Platform.LocalStorageAPI.getItem(
	...
	(async () => {
		...
		while (!Navify.Snackbar?.enqueueCustomSnackbar) {
			await new Promise((resolve) => setTimeout(resolve, 10));
		}

		if (document.querySelector(".Root__globalNav")) {
			const content = Navify.React.createElement("div", {
				dangerouslySetInnerHTML: {
					__html:
						"Sidebar config is not supported when Global Navbar is enabled.<br>In your terminal, please run <code>navify config sidebar_config 0</code> command and then re-apply navify with <code>navify apply</code>.",
				},
				style: {
					"text-size": "12px",
				},
			...
			Navify.Snackbar?.enqueueCustomSnackbar("sidebar-config", {
				keyPrefix: "sidebar-config",
				autoHideDuration: 7500,
				children: Navify.ReactComponent.Snackbar.wrapper({
					children: Navify.React.createElement(Navify.ReactComponent.Snackbar.simpleLayout, {
						center: content,
						children: content,
						dragMetadata: {},
					}),
	...
	function InitSidebarXConfig() {
		...
		appItems.id = "navify-sticky-list";
		// SHOW container
		list = document.createElement("ul");
		list.id = "navify-show-list";
		// HIDDEN container
		hiddenList = document.createElement("ul");
		hiddenList.id = "navify-hidden-list";
		hiddenList.classList.add("hidden-visually");
		const playlistList = libraryItems.querySelector("ul");
		playlistList.id = "navify-playlist-list";
		...
		let storage = [];
		try {
			storage = JSON.parse(localStorage.getItem("navify-sidebar-config:ylx"));
		...
		const observer = new MutationObserver((mutations) => {
			for (const mutation of mutations) {
				if (mutation.type === "childList" && mutation.addedNodes.length) {
					mutation.addedNodes[0].id = "navify-playlist-list";
				}
			}
		});
...
.Root__nav-bar :not(.main-yourLibraryX-entryPoints) > #navify-show-list >* {
padding: 0 24px 0 8px;
}
.main-yourLibraryX-entryPoints #navify-show-list,
.main-yourLibraryX-entryPoints #navify-hidden-list {
padding: 0 12px;
}
`;
