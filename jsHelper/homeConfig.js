...
(async () => {
	...
	NavifyHomeConfig.arrange = (sections) => {
		mounted = true;
		if (list) {
			return list;
		}
		const stickList = (localStorage.getItem("navify-home-config:stick") || "").split(",");
		const lowList = (localStorage.getItem("navify-home-config:low") || "").split(",");
		const stickSections = [];
		const lowSections = [];
...
#navify-home-config button:disabled {
    color: var(--spice-button-disabled);
}
`;

	const container = document.createElement("div");
	container.id = "navify-home-config";
	...
	function injectInteraction() {
		...
		function appendItems() {
			...
			localStorage.setItem(
				"navify-home-config:stick",
				stick.map((a) => a.dataset.uri)
			);
			localStorage.setItem(
				"navify-home-config:low",
	...
	NavifyHomeConfig.menu = new Navify.Menu.Item(
		"Home config",
		false,
		(self) => {
		...
		Navify.SVGIcons["grid-view"]
	);

	NavifyHomeConfig.addToMenu = () => {
		NavifyHomeConfig.menu.register();
	};
	NavifyHomeConfig.removeMenu = () => {
		NavifyHomeConfig.menu.setState(false);
		NavifyHomeConfig.menu.deregister();
	};

	await new Promise((res) => Navify.Events.platformLoaded.on(res));
	// Init
	if (Navify.Platform.History.location.pathname === "/") {
		NavifyHomeConfig.addToMenu();
	}

	Navify.Platform.History.listen(({ pathname }) => {
		if (pathname === "/") {
			NavifyHomeConfig.addToMenu();
		} else {
			NavifyHomeConfig.removeMenu();
