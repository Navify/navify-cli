const CreditFooter = react.memo(({ provider, copyright }) => {
	...
	const credit = [Navify.Locale.get("web-player.lyrics.providedBy", provider)];
	if (copyright) {
...
const SyncedLyricsPage = react.memo(({ lyrics = [], provider, copyright, isKara }) => {
	...
	useTrackPosition(() => {
		const newPos = Navify.Player.getProgress();
						...
						: react.createElement(
								"p",
								{
									onContextMenu: (event) => {
										...
										Navify.Platform.ClipboardAPI.copy(Utils.convertParsedToLRC(lyrics, belowMode).original)
											.then(() => Navify.showNotification("Lyrics copied to clipboard"))
											.catch(() => Navify.showNotification("Failed to copy lyrics to clipboard"));
									},
								},
					...
					belowMode &&
						react.createElement(
							"p",
							{
								...
								onContextMenu: (event) => {
									event.preventDefault();
									Navify.Platform.ClipboardAPI.copy(Utils.convertParsedToLRC(lyrics, belowMode).conver)
										.then(() => Navify.showNotification("Translated lyrics copied to clipboard"))
										.catch(() => Navify.showNotification("Failed to copy translated lyrics to clipboard"));
								},
							},
							text
						)
...
class SearchBar extends react.Component {
	...
	componentDidMount() {
		...
		this.toggleCallback = () => {
			if (!(Navify.Platform.History.location.pathname === "/lyrics-plus" && this.container)) return;
		...
		Navify.Mousetrap().bind("mod+shift+f", this.toggleCallback);
		Navify.Mousetrap(this.container).bind("mod+shift+f", this.toggleCallback);
		Navify.Mousetrap(this.container).bind("enter", this.loopThroughCallback);
		Navify.Mousetrap(this.container).bind("shift+enter", this.loopThroughCallback);
		Navify.Mousetrap(this.container).bind("esc", this.unFocusCallback);
	}

	componentWillUnmount() {
		Navify.Mousetrap().unbind("mod+shift+f", this.toggleCallback);
		Navify.Mousetrap(this.container).unbind("mod+shift+f", this.toggleCallback);
		Navify.Mousetrap(this.container).unbind("enter", this.loopThroughCallback);
		Navify.Mousetrap(this.container).unbind("shift+enter", this.loopThroughCallback);
		Navify.Mousetrap(this.container).unbind("esc", this.unFocusCallback);
	}
...
const SyncedExpandedLyricsPage = react.memo(({ lyrics, provider, copyright, isKara }) => {
	const [position, setPosition] = useState(() => Navify.Player.getProgress() + CONFIG.visual["global-delay"] + CONFIG.visual.delay);
	const activeLineRef = useRef(null);
	const pageRef = useRef(null);

	useTrackPosition(() => {
		if (!Navify.Player.data.is_paused) {
			setPosition(Navify.Player.getProgress() + CONFIG.visual["global-delay"] + CONFIG.visual.delay);
		}
	});
			...
			return react.createElement(
				...
				react.createElement(
					"p",
					{
						onContextMenu: (event) => {
							event.preventDefault();
							Navify.Platform.ClipboardAPI.copy(Utils.convertParsedToUnsynced(lyrics, belowMode).original)
								.then(() => Navify.showNotification("Lyrics copied to clipboard"))
								.catch(() => Navify.showNotification("Failed to copy lyrics to clipboard"));
						},
					},
				...
				belowMode &&
					react.createElement(
						"p",
						{
							...
							onContextMenu: (event) => {
								event.preventDefault();
								Navify.Platform.ClipboardAPI.copy(Utils.convertParsedToUnsynced(lyrics, belowMode).conver)
									.then(() => Navify.showNotification("Translated lyrics copied to clipboard"))
									.catch(() => Navify.showNotification("Failed to copy translated lyrics to clipboard"));
							},
						},
		...
		const lyricsEl1 = react.createElement(
			...
			react.createElement("div", {
				...
				onContextMenu: (event) => {
					...
					Navify.Platform.ClipboardAPI.copy(copylyrics)
						.then(() => Navify.showNotification("Lyrics copied to clipboard"))
						.catch(() => Navify.showNotification("Failed to copy lyrics to clipboard"));
				},
			})
