(function PlaybarButton() {
	if (!Navify.Platform.History) {
		setTimeout(PlaybarButton, 300);
		return;
	}

	const button = new Navify.Playbar.Button(
		"Lyrics Plus",
		...
		() =>
			Navify.Platform.History.location.pathname !== "/lyrics-plus"
				? Navify.Platform.History.push("/lyrics-plus")
				: Navify.Platform.History.goBack(),
		false,
		Navify.Platform.History.location.pathname === "/lyrics-plus",
		false
	);
	...
	if (Navify.LocalStorage.get("lyrics-plus:visual:playbar-button") === "true") setPlaybarButton();
	...
	Navify.Platform.History.listen((location) => {
