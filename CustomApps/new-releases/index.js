...
let dismissed;
try {
	dismissed = JSON.parse(Navify.LocalStorage.get("new-releases:dismissed"));
...
class Grid extends react.Component {
	...
	removeCards(id, type) {
		switch (type) {
			case "reset":
				Navify.showNotification("Reset dismissed releases");
				dismissed = [];
				break;
			case "undo":
				if (!dismissed[0]) Navify.showNotification("Nothing to undo", true);
				else Navify.showNotification("Undone dismissal");
		...
		Navify.LocalStorage.set("new-releases:dismissed", JSON.stringify(dismissed));
	...
	async componentDidMount() {
		...
		this.configButton = new Navify.Menu.Item(
			"New Releases config",
			false,
			openConfig,
	...
	render() {
		...
		const version = Navify.Platform.version.split(".").map((i) => Number.parseInt(i));
		...
		return react.createElement(
			...
			react.createElement(
				...
				react.createElement("h1", null, Navify.Locale.get("new_releases")),
				react.createElement(
					"div",
					{
					...
					react.createElement(ButtonText, {
						text: Navify.Locale.get("playlist.extender.refresh"),
...
async function getArtistList() {
	...
	const artists = await Navify.Platform.LibraryAPI.getContents(config);
	count(true);
	return artists.items ?? [];
}

async function getArtistEverything(artist) {
	const { data, errors } = await Navify.GraphQL.Request(
		{
	...
	const items = [];
	const types = [
		[CONFIG.album, releases.filter((r) => r.type === "ALBUM"), Navify.Locale.get("album")],
		...
		// [CONFIG["appears-on"], releases.appears_on?.releases, Navify.Locale.get("artist.appears-on")],
		[CONFIG.compilations, releases.filter((r) => r.type === "COMPILATION"), Navify.Locale.get("compilation")],
		[
			CONFIG["single-ep"],
			releases.filter((r) => r.type === "SINGLE" || r.type === "EP"),
			`${Navify.Locale.get("single")}/${Navify.Locale.get("ep")}`,
		],
	];
...
async function getPodcastList() {
	const body = await Navify.Platform.LibraryAPI.getShows({ limit: 50000 });
	return body.items ?? [];
}

async function getPodcastRelease(uri) {
	const body = await Navify.Platform.ShowAPI.getContents(uri, { limit: 50000 });
	return body.items;
}
...
async function fetchTracks() {
	...
	Navify.showNotification(`Fetching releases from ${artistList.length} artists`);
...
async function fetchPodcasts() {
	const items = [];
	const itemTypeStr = Navify.Locale.get("card.tag.episode");
