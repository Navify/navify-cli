function DraggableComponent({ uri, title, children }) {
	const dragHandler = Navify.ReactHook.DragHandler?.([uri], title);
	return dragHandler
...
class Card extends react.Component {
	constructor(props) {
		...
		const uriType = Navify.URI.fromString(this.uri)?.type;
		switch (uriType) {
			case Navify.URI.Type.ALBUM:
			case Navify.URI.Type.TRACK:
				this.menuType = Navify.ReactComponent.AlbumMenu;
				break;
		}
	...
	play(event) {
		Navify.Player.playUri(this.uri, this.context);
	...
	closeButtonClicked(event) {
		...
		Navify.Snackbar.enqueueCustomSnackbar
			? Navify.Snackbar.enqueueCustomSnackbar("dismissed-release", {
					keyPrefix: "dismissed-release",
					children: Navify.ReactComponent.Snackbar.wrapper({
						children: Navify.ReactComponent.Snackbar.simpleLayout({
							leading: Navify.ReactComponent.Snackbar.styledImage({
								src: this.props.imageURL,
								imageHeight: "24px",
								imageWidth: "24px",
							}),
							center: Navify.React.createElement("div", {
								dangerouslySetInnerHTML: {
									__html: `Dismissed <b>${this.title}</b>.`,
								},
							}),
							trailing: Navify.ReactComponent.Snackbar.ctaText({
								ctaText: "Undo",
			...
			: Navify.showNotification(`Dismissed <b>${this.title}</b> from <br>${this.artist.name}</b>`);
	}
	...
	render() {
		...
		if (this.visual.count && this.trackCount) {
			detail.push(Navify.Locale.get("tracklist-header.songs-counter", this.trackCount));
		}

		return react.createElement(
			Navify.ReactComponent.RightClickMenu || "div",
			{
							...
							react.createElement(
								...
								react.createElement(
									"div",
									{
										...
										"aria-label": Navify.Locale.get("play"),
										style: { "--size": "40px" },
