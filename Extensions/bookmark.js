...
(function Bookmark() {
	const { CosmosAsync, Player, LocalStorage, ContextMenu, URI } = Navify;
	if (!(CosmosAsync && URI)) {
	...
	// Local Storage keys
	const STORAGE_KEY = "bookmark_navify";
...
`;
			...
			Navify.Tippy(this.querySelectorAll("[data-tippy-content]"), Navify.TippyProps);
			if (isPlayable) {
	...
	function createMenuItem(title, callback) {
		...
		Navify.ReactDOM.render(
			Navify.React.createElement(
				Navify.ReactComponent.MenuItem,
				{
					onClick: () => {
						callback?.();
					},
	...
	async function storeThisPage() {
		...
		const context = Navify.Platform.History.location.pathname;
		try {
			contextUri = Navify.URI.fromString(context);
		} catch (e) {
			Navify.showNotification("Cannot bookmark this page", true);
			return;
		}
	...
	function getTrackMeta() {
		...
		const playerState = Navify.Player.data;
...
`;
	...
	async function onLinkClick(info) {
		if (info.context?.startsWith("/")) {
			Navify.Platform.History.push(info.context);
			return;
		}
		const url = Navify.URI.fromString(info.uri).toURLPath(true);
		Navify.Platform.History.push(url);
	}
	...
	function onPlayClick(info) {
		...
		Navify.Player.playUri(uri, {}, options);
	}

	const fetchAlbum = async (uri) => {
		const { getAlbum } = Navify.GraphQL.Definitions;
		const { data } = await Navify.GraphQL.Request(getAlbum, {
			uri,
	...
	const fetchArtist = async (uri) => {
		...
		const { data } = await Navify.GraphQL.Request(queryArtistOverview, {
			uri,
			locale: Navify.Locale.getLocale(),
	...
	const fetchTrack = async (uri, uid, context) => {
		...
		let newContext;
		if (context && uid && Navify.URI.isPlaylistV1OrV2(context)) {
			newContext = `${Navify.URI.fromString(context).toURLPath(true)}?uid=${uid}`;
		}
		return {
			uri,
			title: res.name,
	...
	const fetchPlaylist = async (uri) => {
		const res = await Navify.CosmosAsync.get(`sp://core-playlist/v1/playlist/${uri}/metadata`, {
	...
	new Navify.ContextMenu.Item(
		...
		async ([uri], [uid] = [], context = undefined) => {
			...
			let meta;
			switch (type) {
				case Navify.URI.Type.TRACK:
					meta = await fetchTrack(uri, uid, context);
					break;
				case Navify.URI.Type.ALBUM:
					meta = await fetchAlbum(uri);
					break;
				case Navify.URI.Type.ARTIST:
					meta = await fetchArtist(uri);
					break;
				case Navify.URI.Type.SHOW:
					meta = await fetchShow(uri);
					break;
				case Navify.URI.Type.EPISODE:
					meta = await fetchEpisode(uri);
					break;
				case Navify.URI.Type.PLAYLIST:
				case Navify.URI.Type.PLAYLIST_V2:
