...
(function KeyboardShortcut() {
	...
	const binds = {
		...
		"ctrl+q": {
			callback: () =>
				Navify.CosmosAsync.post("sp://esperanto/spotify.desktop.lifecycle_esperanto.proto.DesktopLifecycle/Shutdown") &&
				Navify.CosmosAsync.post("sp://desktop/v1/shutdown"),
		},
		...
		"shift+h": { callback: () => Navify.Platform.History.goBack() },
		"shift+l": { callback: () => Navify.Platform.History.goForward() },

		// M to Like/Unlike track
		m: { callback: () => Navify.Player.toggleHeart() },

		// Forward Slash to open search page
		"/": { callback: () => Navify.Platform.History.replace("/search") },

		// CTRL + Arrow Left Next and CTRL + Arrow Right  Previous Song
		"ctrl+left": { callback: () => Navify.Player.back() },
		"ctrl+right": { callback: () => Navify.Player.next() },

		// CTRL + Arrow Up Increase Volume CTRL + Arrow Down Decrease Volume
		"ctrl+up": { callback: () => Navify.Player.setVolume(Navify.Player.getVolume() + 0.05) },
		"ctrl+down": { callback: () => Navify.Player.setVolume(Navify.Player.getVolume() - 0.05) },
	...
	for (const [key, { staticCondition, callback }] of Object.entries(binds)) {
		if (typeof staticCondition === "undefined" || staticCondition) {
			Navify.Mousetrap.bind(key, (event) => {
	...
	function findActiveIndex(allItems) {
		...
		const historyURI = Navify.Platform.History.location.pathname.replace(/^\//, "spotify:").replace(/\//g, ":");
	...
	function rotateSidebar(direction) {
		const allItems = document.querySelectorAll(
			"#navify-sticky-list .main-yourLibraryX-navLink, .main-yourLibraryX-listItem > div:not(:has([data-skip-in-keyboard-nav])) > div:first-child"
		);
...
.vim-key {
	...
	const mousetrap = new Navify.Mousetrap(document);
	...
	/**
	 *
	 * @param {Navify.Keyboard.ValidKey} key
	 */
	this.setCancelKey = function (key) {
		mousetrap.bind(Navify.Keyboard.KEYS[key], this.deactivate.bind(this));
	};
