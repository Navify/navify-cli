...
(async function shufflePlus() {
	if (!(Navify.CosmosAsync && Navify.Platform)) {
	...
	function getConfig() {
		try {
			const parsed = JSON.parse(Navify.LocalStorage.get("shufflePlus:settings"));
			if (parsed && typeof parsed === "object") {
				return parsed;
			}
			throw "";
		} catch {
			Navify.LocalStorage.set("shufflePlus:settings", "{}");
			return {
				artistMode: "all",
	...
	function saveConfig() {
		Navify.LocalStorage.set("shufflePlus:settings", JSON.stringify(CONFIG));
	}
	...
	new Navify.Menu.Item("Shuffle+", false, settingsPage, "shuffle").register();

	const { Type } = Navify.URI;

	function shouldAddShufflePlus(uri) {
		if (uri.length === 1) {
			const uriObj = Navify.URI.fromString(uri[0]);
			switch (uriObj.type) {
				case Type.PLAYLIST:
	...
	function shouldAddShufflePlusLiked(uri) {
		const uriObj = Navify.URI.fromString(uri[0]);
		if (Navify.Platform.History.location.pathname === "/collection/tracks") {
			switch (uriObj.type) {
				case Type.TRACK:
					return true;
			}
	...
	function shouldAddShufflePlusLocal(uri) {
		const uriObj = Navify.URI.fromString(uri[0]);
		if (Navify.Platform.History.location.pathname === "/collection/local-files") {
			switch (uriObj.type) {
				case Type.TRACK:
	...
	function renderQueuePlaybarButton() {
		if (!playbarButton) {
			playbarButton = new Navify.Playbar.Button(
	...
	async function fetchPlaylistTracks(uri) {
		const res = await Navify.Platform.PlaylistAPI.getContents(`spotify:playlist:${uri}`, {
			limit: 9999999,
		});
	...
	async function fetchFolderTracks(uri) {
		const res = await Navify.Platform.RootlistAPI.getContents();
		...
		async function fetchNested(folder) {
			...
			for (const i of folder.items) {
				if (i.type === "playlist") {
					const uriObj = Navify.URI.fromString(i.uri);
	...
	async function fetchAlbumTracks(uri, includeMetadata = false) {
		const { queryAlbumTracks } = Navify.GraphQL.Definitions;
		const { data, errors } = await Navify.GraphQL.Request(queryAlbumTracks, {
			uri,
			offset: 0,
			limit: 100,
		});
	...
	async function scanForTracksFromAlbums(res, artistName, type) {
		...
		for (const album of res) {
			...
			Navify.showNotification(`${artistFetchTypeCount[type]} / ${res.length} ${type}s`);
	...
	async function fetchArtistTracks(uri) {
		...
		const discography = await Navify.GraphQL.Request(queryArtistDiscographyAll, {
			uri,
			offset: 0,
		...
		const overview = await Navify.GraphQL.Request(queryArtistOverview, {
			uri,
			locale: Navify.Locale.getLocale(),
	...
	async function fetchArtistLikedTracks(uri) {
		const artistRes = await Navify.CosmosAsync.get(`sp://core-collection/unstable/@/list/tracks/artist/${uri}?responseFormat=protobufJson`);
	...
	async function fetchArtistTopTenTracks(uri) {
		const { queryArtistOverview } = Navify.GraphQL.Definitions;
		const { data, errors } = await Navify.GraphQL.Request(queryArtistOverview, {
			uri,
			locale: Navify.Locale.getLocale(),
			includePrerelease: false,
		});
	...
	async function fetchLikedTracks() {
		const limit = 9999999;
		const res = await Navify.Platform.LibraryAPI.getTracks({ limit });

		return res.items.filter((track) => track.isPlayable).map((track) => track.uri);
	}

	async function fetchLocalTracks() {
		const res = await Navify.Platform.LocalFilesAPI.getTracks();

		return res.map((track) => track.uri);
	}

	function fetchQueue() {
		const { _queueState } = Navify.Platform.PlayerAPI._queue;
	...
	async function fetchCollection(uriObj) {
		...
		const { pathname } = Navify.Platform.History.location;
	...
	async function fetchShows(uri) {
		const res = await Navify.CosmosAsync.get(`sp://core-show/v1/shows/${uri}?responseFormat=protobufJson`);
	...
	async function Queue(list, context, type) {
		...
		const { _queue, _client } = Navify.Platform.PlayerAPI._queue;
		...
		if (context) {
			const { sessionId } = Navify.Platform.PlayerAPI.getState();
			Navify.Platform.PlayerAPI.updateContext(sessionId, {
				uri: context,
				url: `context://${context}`,
			});
		}
		...
		switch (type) {
			case Type.ARTIST:
				if (CONFIG.artistMode === "topTen") {
					Navify.showNotification(`Shuffled Top ${count} Songs`);
					break;
				}
				if (CONFIG.artistMode === "likedSongArtist") {
					Navify.showNotification(`Shuffled ${count} Liked Songs`);
					break;
				}
				if (CONFIG.artistMode === "single") {
					Navify.showNotification(`Shuffled ${artistFetchTypeCount.single} Singles, Total of ${count} Songs`);
					break;
				}
				if (CONFIG.artistMode === "album") {
					Navify.showNotification(`Shuffled ${artistFetchTypeCount.album} Albums, Total of ${count} Songs`);
					break;
				}
				Navify.showNotification(`Shuffled ${artistFetchTypeCount.album} Albums, ${artistFetchTypeCount.single} Singles, Total of ${count} Songs`);
				break;
			default:
				Navify.showNotification(`Shuffled ${count} Songs`);
		}
	...
	async function fetchAndPlay(rawUri) {
		...
		try {
			if (rawUri === "queue") {
				...
				const uriObj = Navify.URI.fromString(rawUri);
				type = uriObj.type;
				...
				if (!list?.length) {
					Navify.showNotification("Nothing to play", true);
					return;
				}
			...
			Navify.showNotification(String(error), true);
			console.error(error);
		}
	}
})();
