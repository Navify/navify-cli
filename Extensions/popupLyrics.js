...
function PopupLyrics() {
	const { Player, CosmosAsync, LocalStorage, ContextMenu } = Navify;
	...
	const button = new Navify.Topbar.Button("Popup Lyrics", "lyrics", () => {
		if (!lyricVideoIsOpen) {
	...
	async function updateTrack(refresh = false) {
		...
		if (!Navify.URI.isTrack(Player.data.item.uri) && !Navify.URI.isLocalTrack(Player.data.item.uri)) {
			return;
		}
...
<div class="setting-row">
	...
	function musixmatchTokenElements(defaultVal, id) {
		...
		function clickRefresh() {
			...
			Navify.CosmosAsync.get("https://apic-desktop.musixmatch.com/ws/1.1/token.get?app_id=web-desktop-app-v1.0", null, {
				authority: "apic-desktop.musixmatch.com",
			})
    ...
    <div class="col action">
        <button class="switch small">
            <svg height="10" width="10" viewBox="0 0 16 16" fill="currentColor">
                ${Navify.SVGIcons["chart-up"]}
            </svg>
        </button>
        <button class="switch small">
            <svg height="10" width="10" viewBox="0 0 16 16" fill="currentColor">
                ${Navify.SVGIcons["chart-down"]}
            </svg>
        </button>
        <button class="switch">
            <svg height="16" width="16" viewBox="0 0 16 16" fill="currentColor">
