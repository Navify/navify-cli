...
(function TrashBin() {
	...
	if (!Navify.Player.data || !Navify.LocalStorage || !skipBackBtn) {
	...
	function createSlider(name, desc, defaultVal, callback) {
		...
		slider.onclick = () => {
			...
			Navify.LocalStorage.set(name, state);
	...
	function settingsContent() {
		...
		content.appendChild(
			createButton("Clear ", "Clear all items from trashbin (cannot be reverted).", () => {
				...
				putDataLocal();
				Navify.showNotification("Trashbin cleared!");
			})
		);
	}
	...
	function initValue(item, defaultValue) {
		try {
			const value = JSON.parse(Navify.LocalStorage.get(item));
	...
	const widget = new Navify.Playbar.Widget(
		THROW_TEXT,
		trashbinIcon,
		(self) => {
			const uri = Navify.Player.data.item.uri;
			const uriObj = Navify.URI.fromString(uri);
			const type = uriObj.type;

			if (!trashSongList[uri]) {
				trashSongList[uri] = true;
				if (shouldSkipCurrentTrack(uri, type)) Navify.Player.next();
				Navify.showNotification("Song added to trashbin");
			} else {
				delete trashSongList[uri];
				setWidgetState(false);
				Navify.showNotification("Song removed from trashbin");
			}
	...
	setWidgetState(
		trashSongList[Navify.Player.data.item.uri],
		Navify.URI.fromString(Navify.Player.data.item.uri).type !== Navify.URI.Type.TRACK
	);
	...
	function refreshEventListeners(state) {
		...
		if (state) {
			...
			Navify.Player.addEventListener("songchange", watchChange);
			enableWidget && widget.register();
			watchChange();
		} else {
			skipBackBtn.removeEventListener("click", eventListener);
			Navify.Player.removeEventListener("songchange", watchChange);
			widget.deregister();
		}
	}
	...
	function watchChange() {
		...
		setWidgetState(isBanned, Navify.URI.fromString(data.item.uri).type !== Navify.URI.Type.TRACK);
	...
	function shouldSkipCurrentTrack(uri, type) {
		const curTrack = Navify.Player.data.item;
		if (type === Navify.URI.Type.TRACK) {
			if (uri === curTrack.uri) {
				return true;
			}
		}

		if (type === Navify.URI.Type.ARTIST) {
			let count = 1;
	...
	function toggleThrow(uris) {
		...
		const list = type === Navify.URI.Type.TRACK ? trashSongList : trashArtistList;

		if (!list[uri]) {
			list[uri] = true;
			if (shouldSkipCurrentTrack(uri, type)) Navify.Player.next();
			Navify.Player.data?.item.uri === uri && setWidgetState(true);
			Navify.showNotification(type === Navify.URI.Type.TRACK ? "Song added to trashbin" : "Artist added to trashbin");
		} else {
			delete list[uri];
			Navify.Player.data?.item.uri === uri && setWidgetState(false);
			Navify.showNotification(type === Navify.URI.Type.TRACK ? "Song removed from trashbin" : "Artist removed from trashbin");
		}
	...
	function shouldAddContextMenu(uris) {
		...
		const uri = uris[0];
		const uriObj = Navify.URI.fromString(uri);
		if (uriObj.type === Navify.URI.Type.TRACK) {
			cntxMenu.name = trashSongList[uri] ? UNTHROW_TEXT : THROW_TEXT;
			return true;
		}

		if (uriObj.type === Navify.URI.Type.ARTIST) {
	...
	const cntxMenu = new Navify.ContextMenu.Item(THROW_TEXT, toggleThrow, shouldAddContextMenu, trashbinIcon);
	cntxMenu.register();

	function putDataLocal() {
		Navify.LocalStorage.set("TrashSongList", JSON.stringify(trashSongList));
		Navify.LocalStorage.set("TrashArtistList", JSON.stringify(trashArtistList));
	}
	...
	function copyItems() {
		...
		Navify.Platform.ClipboardAPI.copy(JSON.stringify(data));
		Navify.showNotification("Copied to clipboard");
	}
	...
	async function exportItems() {
		...
		try {
			const handle = await window.showSaveFilePicker({
				suggestedName: "navify-trashbin.json",
				types: [
					{
						description: "Navify trashbin backup",
						accept: {
			...
			Navify.showNotification("Backup saved succesfully.");
		} catch {
			Navify.showNotification("Failed to save, try copying trashbin contents to clipboard and creating a backup manually.");
		}
	}
		...
		input.onchange = (e) => {
			...
			reader.onload = (e) => {
				try {
					...
					putDataLocal();
					Navify.showNotification("File Import Successful!");
				} catch (e) {
					Navify.showNotification("File Import Failed!", true);
					console.error(e);
				}
			};
			reader.onerror = () => {
				Navify.showNotification("File Read Failed!", true);
