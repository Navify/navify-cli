...
(function FullAppDisplay() {
	if (!Navify.Keyboard || !Navify.React || !Navify.ReactDOM) {
		setTimeout(FullAppDisplay, 200);
		return;
	}

	const { React: react, ReactDOM: reactDOM } = Navify;
...
#fad-art {
	...
	const ProgressBar = () => {
		const [progress, setProgress] = useState(Navify.Player.getProgress());
		const duration = Navify.Platform.PlayerAPI._state.duration;
		...
		useEffect(() => {
			...
			Navify.Player.addEventListener("onprogress", update);
			return () => Navify.Player.removeEventListener("onprogress", update);
		}, [isDragging]);
		...
		const handleClick = (e) => {
			...
			Navify.Player.seek(newProgress);
		...
		return react.createElement(
			...
			react.createElement("span", { id: "fad-elapsed" }, Navify.Player.formatTime(progress)),
			react.createElement(
				"div",
				{
					id: "fad-progress",
			...
			react.createElement("span", { id: "fad-duration" }, Navify.Player.formatTime(duration))
		);
	};

	const PlayerControls = () => {
		const [value, setValue] = useState(Navify.Player.isPlaying());
		useEffect(() => {
			const update = ({ data }) => setValue(!data.isPaused);
			Navify.Player.addEventListener("onplaypause", update);
			return () => Navify.Player.removeEventListener("onplaypause", update);
		});
		return react.createElement(
			"div",
			{ id: "fad-controls" },
			react.createElement(ButtonIcon, {
				icon: Navify.SVGIcons["skip-back"],
				onClick: Navify.Player.back,
			}),
			react.createElement(ButtonIcon, {
				icon: Navify.SVGIcons[value ? "pause" : "play"],
				onClick: Navify.Player.togglePlay,
			}),
			react.createElement(ButtonIcon, {
				icon: Navify.SVGIcons["skip-forward"],
				onClick: Navify.Player.next,
			})
		);
	};
	...
	class FAD extends react.Component {
		constructor(props) {
			...
			this.state = {
				...
				releaseDate: "",
				cover: "",
				heart: Navify.Player.getHeart(),
			};
			this.currTrackImg = new Image();
			this.nextTrackImg = new Image();
			this.mousetrap = new Navify.Mousetrap();
		}

		async getAlbumDate(uri) {
			const { getAlbum } = Navify.GraphQL.Definitions;
			const { errors, data } = await Navify.GraphQL.Request(getAlbum, {
				uri,
		...
		async fetchInfo() {
			const meta = Navify.Player.data.item.metadata;
		...
		componentDidMount() {
			...
			Navify.Player.addEventListener("songchange", this.updateInfo);
			this.updateInfo();
			...
			Navify.Platform.PlayerAPI._events.addListener("queue_update", this.onQueueChange);
			this.mousetrap.bind("esc", deactivate);
			window.dispatchEvent(new Event("fad-request"));
		}

		componentWillUnmount() {
			Navify.Player.removeEventListener("songchange", this.updateInfo);
			Navify.Platform.PlayerAPI._events.removeListener("queue_update", this.onQueueChange);
					...
					react.createElement(
								...
								react.createElement(
									...
									react.createElement(
										...
										react.createElement(DisplayIcon, {
											icon: Navify.SVGIcons[this.state.heart ? "heart-active" : "heart"],
											size: 50,
										})
									)
								),
						...
						react.createElement(
							...
							react.createElement(SubInfo, {
								...
								icon: Navify.SVGIcons.artist,
							}),
							CONFIG.showAlbum &&
								react.createElement(SubInfo, {
									id: "fad-album",
									text: this.state.album,
									icon: Navify.SVGIcons.album,
								}),
							CONFIG.showReleaseDate &&
								react.createElement(SubInfo, {
									id: "fad-release-date",
									text: this.state.releaseDate,
									icon: Navify.SVGIcons.clock,
								}),
							react.createElement(
								"div",
								{
	...
	function deactivate() {
		...
		if (lastApp && lastApp !== "/lyrics-plus") {
			Navify.Platform.History.push(lastApp);
		}
	}
	...
	function checkLyricsPlus() {
		return Navify.Config?.custom_apps?.includes("lyrics-plus") || !!document.querySelector("a[href='/lyrics-plus']");
	}

	function requestLyricsPlus() {
		if (CONFIG.lyricsPlus && checkLyricsPlus()) {
			lastApp = Navify.Platform.History.location.pathname;
			if (lastApp !== "/lyrics-plus") {
				Navify.Platform.History.push("/lyrics-plus");
			}
		}
		window.dispatchEvent(new Event("fad-request"));
	}
	...
	function getConfig() {
		try {
			const parsed = JSON.parse(Navify.LocalStorage.get("full-app-display-config") || "{}");
			if (parsed && typeof parsed === "object") {
				return parsed;
			}
			throw "";
		} catch {
			Navify.LocalStorage.set("full-app-display-config", "{}");
			return {};
		}
	}

	function saveConfig() {
		Navify.LocalStorage.set("full-app-display-config", JSON.stringify(CONFIG));
	}
...
`,
	...
	new Navify.Topbar.Button(
		"Full App Display",
		`<svg role="img" height="16" width="16" viewBox="0 0 16 16" fill="currentColor">${Navify.SVGIcons.projector}</svg>`,
		activate
	);

	Navify.Mousetrap.bind("f11", toggleFad);
})();
