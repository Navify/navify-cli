...
`;
	...
	function saveState() {
		...
		if (!uri) return;
		Navify.LocalStorage.set(`loopyLoop:${uri}`, JSON.stringify({ start, end, skipZones }));
	}
	...
	function loadState() {
		...
		if (!uri) return;
		try {
			const saved = Navify.LocalStorage.get(`loopyLoop:${uri}`);
			if (saved) {
	...
	function applyMoveAdjustment(deltaSeconds) {
		const durationMs = Navify.Player.getDuration();
	...
	Navify.Player.addEventListener("onprogress", (event) => {
		...
		const durationMs = Navify.Player.getDuration() || 0;
		...
		if (prevProgressPercent > threeSecFrac && percent < nearZeroFrac) {
			if (prevPressedAt > 0 && ts - prevPressedAt < 1500) {
				...
				prevPressedAt = ts;
				prevProgressPercent = percent;
				if (start !== null) Navify.Player.seek(start);
				return;
			}
		}
		prevProgressPercent = percent;
		...
		if (end !== null && percent >= end) {
			// Navify.Player.getRepeat(): 0 = off, 1 = repeat context, 2 = repeat track
			if (Navify.Player.getRepeat() === 2) {
				if (ts - lastEndLoopSeek > 500) {
					lastEndLoopSeek = ts;
					Navify.Player.seek(start ?? 0);
				}
			} else if (ts - lastNextCall > 2000) {
				lastNextCall = ts;
				seekStartPendingUri = Navify.Player.data?.item?.uri ?? null;
				Navify.Player.next();
			}
			return;
		}
	...
	Navify.Player.addEventListener("songchange", () => {
		...
		if (Navify.Player.data?.item?.uri !== seekStartPendingUri) seekStartPendingUri = null;
		loadState();
		drawOnBar();
		drawSkipMarkers();
	...
	const startBtn = createMenuItem("Set song start", () => {
		if (end !== null && mouseOnBarPercent >= end) {
			Navify.showNotification("Song start must be before song end");
			return;
		}
	...
	const endBtn = createMenuItem("Set song end", () => {
		if (start !== null && mouseOnBarPercent <= start) {
			Navify.showNotification("Song end must be after song start");
			return;
		}
	...
	const skipEndBtn = createMenuItem("Set section skip end", () => {
		if (pendingSkipStart === null) {
			Navify.showNotification("No section skip start selected!");
			return;
		}
		...
		if (e > s) {
			if (skipZones.length < 10) {
				...
				Navify.showNotification("Maximum 10 skip zones reached");
			}
		}
		pendingSkipStart = null;
	});
	...
	function tryLoadInitialState(attemptsLeft) {
		if (Navify.Player.data?.item?.uri) {
			loadState();
			drawOnBar();
			drawSkipMarkers();
	...
	try {
		...
		const toolbarBtn = new Navify.Playbar.Button("Loopy Loop", markerIcon, () => {
			mouseOnBarPercent = Navify.Player.getProgressPercent();
