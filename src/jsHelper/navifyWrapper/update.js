...
void (async function checkForUpdate() {
  ...
  const { check_navify_update, version } = Navify.Config;
  // Skip checking if upgrade check is disabled, or version is Dev/version is not set
  if (!check_navify_update || !version || version === "Dev") return;
  // Fetch latest version from GitHub
  try {
    let changelog;
    const res = await fetch("https://api.github.com/repos/Navify/navify-cli/releases/latest");
    ...
    if (semver !== version) {
      ...
      content.innerHTML = `
				<style>
					...
					.navify-update-little-space {
						margin-bottom: 8px;
					}
				</style>
				<p class="navify-update-space">Update Navify to receive new features and bug fixes.</p>
				...
				<ol>
					<li>Update Navify CLI</li>
					<pre class="navify-update-little-space">navify update</pre>
					<p>Navify will automatically apply changes to Spotify after upgrading to the latest version.</p>
					<p>If you installed Navify via a package manager, update using said package manager.</p>
				</ol>
			`;

      void (async function attachUpdateTippy() {
        const Tippy = await waitFor(() => Navify.Tippy, 300);
        ...
        for (const instance of tippy) {
          instance.reference.addEventListener("click", () => {
            Navify.Platform.ClipboardAPI.copy(instance.reference.textContent);
      ...
      new Navify.Topbar.Button(
        ...
        () => Navify.PopupModal.display(updateModal),
      );
    }
  } catch (err) {
    console.error(err);
