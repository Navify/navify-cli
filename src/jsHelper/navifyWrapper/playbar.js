...
Navify.Playbar = (() => {
  ...
  class Button {
    constructor(label, icon, onClick = () => {}, disabled = false, active = false, registerOnCreate = true) {
      ...
      this.tippy = Navify.Tippy?.(this.element, {
        content: label,
        ...Navify.TippyProps,
      });
      this.label = label;
    ...
    set icon(input) {
      let newInput = input;
      if (newInput && Navify.SVGIcons[newInput]) {
        newInput = `<svg height="16" width="16" viewBox="0 0 16 16" fill="currentColor" stroke="currentColor">${Navify.SVGIcons[newInput]}</svg>`;
      }
      this._icon = newInput;
  ...
  class Widget {
    ...
    set icon(input) {
      let newInput = input;
      if (newInput && Navify.SVGIcons[newInput]) {
        newInput = `<svg height="16" width="16" viewBox="0 0 16 16" fill="currentColor">${Navify.SVGIcons[newInput]}</svg>`;
      }
      this._icon = newInput;
