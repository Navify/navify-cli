...
Navify.ContextMenuV2 = (() => {
  ...
  class Item {
    constructor({ children, disabled = false, leadingIcon, trailingIcon, divider, onClick, shouldAdd = () => true }) {
      ...
      this._element = Navify.ReactJSX.jsx(() => {
        const [_children, setChildren] = Navify.React.useState(this._children);
        const [_disabled, setDisabled] = Navify.React.useState(this._disabled);
        const [_leadingIcon, setLeadingIcon] = Navify.React.useState(this._leadingIcon);
        const [_trailingIcon, setTrailingIcon] = Navify.React.useState(this._trailingIcon);
        const [_divider, setDivider] = Navify.React.useState(this._divider);

        Navify.React.useEffect(() => {
        ...
        const context = Navify.React.useContext(Navify.ContextMenuV2._context) ?? {};
    ...
    register() {
      Navify.ContextMenuV2.registerItem(this._element, this.shouldAdd);
    }
    deregister() {
      Navify.ContextMenuV2.unregisterItem(this._element);
    }
  }
  ...
  class ItemSubMenu {
    ...
    constructor({ text, disabled = false, leadingIcon, divider, items, depth = 1, shouldAdd = () => true }) {
      ...
      this._depth = depth;
      this._element = Navify.ReactJSX.jsx(() => {
        const [_text, setText] = Navify.React.useState(this._text);
        const [_disabled, setDisabled] = Navify.React.useState(this._disabled);
        const [_leadingIcon, setLeadingIcon] = Navify.React.useState(this._leadingIcon);
        const [_divider, setDivider] = Navify.React.useState(this._divider);
        const [_items, setItems] = Navify.React.useState(this._items);
        const [_depth, setDepth] = Navify.React.useState(this._depth);
        ...
        return Navify.React.createElement(
          Navify.ReactComponent.MenuSubMenuItem,
          {
            displayText: _text,
            divider: _divider,
            depth: _depth,
  ...
  const renderItems = () => {
    const { props, trigger, target } = Navify.React.useContext(Navify.ContextMenuV2._context) ?? {};

    return Navify.React.useMemo(
      () =>
...
Navify.Menu = (() => {
  ...
  class Item extends Navify.ContextMenuV2.Item {
  ...
  class SubMenu extends Navify.ContextMenuV2.ItemSubMenu {
...
Navify.ContextMenu = (() => {
  const iconList = Object.keys(Navify.SVGIcons);
  ...
  class Item extends Navify.ContextMenuV2.Item {
    ...
    constructor(name, onClick, shouldAdd, icon, trailingIcon, disabled) {
      ...
      super({
        ...
        trailingIcon,
        onClick: (context) => {
          const [uris, uids, contextUri] = Navify.ContextMenuV2.parseProps(context.props);
          onClick(uris, uids, contextUri);
        },
        shouldAdd: (props) => {
          const parsedProps = Navify.ContextMenuV2.parseProps(props);
