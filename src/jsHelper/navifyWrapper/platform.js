...
(function waitForPlatform() {
  ...
  const { _platform } = Navify;
  for (const key of Object.keys(_platform)) {
    if (key.startsWith("get") && typeof _platform[key] === "function") {
      Navify.Platform[key.slice(3)] = _platform[key]();
    } else {
      Navify.Platform[key] = _platform[key];
    }
  }
})();

(function addMissingPlatformAPIs() {
  if (!Navify.Platform?.version && !Navify.Platform?.Registry) {
    setTimeout(addMissingPlatformAPIs, 50);
    return;
  }
  const os = Navify.Platform.operatingSystem;
  const version = Navify.Platform.version.split(".").map((i) => Number.parseInt(i, 10));
  if (version[0] === 1 && version[1] === 2 && version[2] < 38) return;

  for (const [key, _] of Navify.Platform.Registry._map.entries()) {
    ...
    if (Object.hasOwn(Navify.Platform, symbolName)) continue;
    try {
      const resolvedAPI = Navify.Platform.Registry.resolve(key);
      Navify.Platform[symbolName] = resolvedAPI;

      console.debug(`[navifyWrapper] Resolved PlatformAPI from Registry: ${symbolName}`);
    } catch (err) {
      console.error(`[navifyWrapper] Error resolving PlatformAPI from Registry: ${symbolName}`, err);
    }
  }
})();
...
void (async function addProxyCosmos() {
  if (!Navify.Player.origin?._cosmos && !Navify.Platform?.Registry) {
    setTimeout(addProxyCosmos, 50);
    return;
  }

  const _cosmos = Navify.Player.origin?._cosmos ?? Navify.Platform?.Registry.resolve(Symbol.for("Cosmos"));
  ...
  const handler = {
    get: (target, prop, receiver) => {
      ...
      return async function (url, body) {
        ...
        const corsProxyURLTemplate = window.localStorage.getItem("navify:corsProxyTemplate") ?? "https://cors-proxy.navify.app/{url}";
        ...
        if (shouldUseCORSProxy) {
          ...
          try {
            ...
            console.error("[navifyWrapper] Invalid CORS Proxy URL template");
          }
        }

        const Authorization = `Bearer ${Navify.Platform.AuthorizationAPI.getState().token.accessToken}`;
        let injectedHeaders = {};
        if (isWebAPI) injectedHeaders = { Authorization };
        if (isSpClientAPI) {
          injectedHeaders = {
            Authorization,
            "Spotify-App-Version": Navify.Platform.version,
            "App-Platform": Navify.Platform.PlatformData.app_platform,
          };
        }
  ...
  await waitFor(() => Navify.Player.origin, 50);
  Navify.Player.origin._cosmos = new Proxy(_cosmos, handler);
  Object.defineProperty(Navify, "CosmosAsync", {
    get: () => {
      return Navify.Player.origin?._cosmos;
    },
  });
})();
