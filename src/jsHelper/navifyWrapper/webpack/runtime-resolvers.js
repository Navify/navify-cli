...
export function exposeRuntimeResolvers({ cache, chunks, modules, functionModules, require }) {
  (function waitForSnackbar(attempt = 0) {
    if (!Object.keys(Navify.Snackbar).length) {
  ...
  if (Navify.Locale) Navify.Locale._supportedLocales = cache.find((m) => typeof m?.ja === "string");

  Object.defineProperty(Navify, "Queue", {
    get() {
      return Navify.Player.origin?._queue?._state ?? Navify.Player.origin?._queue?._queue;
    },
  });
  ...
  if (!Navify.ReactComponent?.ConfirmDialog && confirmDialogChunk) {
    Navify.ReactComponent.ConfirmDialog = Object.values(require(confirmDialogChunk[0])).find((m) => typeof m === "object");
  } else {
    Navify.ReactComponent.ConfirmDialog = functionModules.find(
  ...
  if (contextMenuChunk) {
    Navify.ReactComponent.ContextMenu = Object.values(require(contextMenuChunk[0])).find((m) => typeof m === "function");
  }
  ...
  if (infiniteQueryChunk) {
    Navify.ReactQuery.useInfiniteQuery = Object.values(require(infiniteQueryChunk[0])).find((m) => typeof m === "function");
  }

  if (Navify.Color) Navify.Color.CSSFormat = modules.find((m) => m?.RGBA);

  // Combine snackbar and notification
  (function bindShowNotification(attempt = 0) {
    if (!Navify.Snackbar?.enqueueSnackbar && !Navify.showNotification) {
      setTimeout(() => bindShowNotification(attempt + 1), Math.min(250 * 2 ** Math.min(attempt, 4), 1000));
      return;
    }

    if (Navify.Snackbar?.enqueueSnackbar) {
      Navify.showNotification = (message, isError, msTimeout) => {
        Navify.Snackbar.enqueueSnackbar(message, {
    ...
    Navify.Snackbar.enqueueSnackbar = (message, { variant = "default", autoHideDuration } = {}) => {
      isError = variant === "error";
      Navify.showNotification(message, isError, autoHideDuration);
    };
  })();

  // Image color extractor
  void (async function bindColorExtractor(attempt = 0) {
    if (!Navify.GraphQL.Request) {
    ...
    Navify.extractColorPreset = async (image) => {
      const analysis = await imageAnalysis(Navify.GraphQL.Request, image);
