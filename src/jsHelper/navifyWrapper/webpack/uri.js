...
export function waitForURI(cache) {
  ...
  (function waitForURI(attempt = 0) {
    ...
    if (Navify.URI.Type && Navify.URI.from && Navify.URI.fromString && Navify.URI.idToHex && Navify.URI.hexToId) return;
    ...
    Navify.URI.Type ??= getURIType(URIExports);
    if (!Navify.URI.Type) {
      setTimeout(() => waitForURI(attempt + 1), Math.min(100 * 2 ** Math.min(attempt, 4), 1000));
      return;
    }
    ...
    Navify.URI.fromString ??= findURIFromString(URIExports, allFunctions);
    Navify.URI.from ??= findURIFrom(URIExports, allFunctions) ?? Navify.URI.fromString;

    // createURI functions
    const createURIFunctions = URIModules.filter((m) => fnStr(m).match(/\([\w$]+\./));
    for (const type of Object.keys(Navify.URI.Type)) {
      const func = createURIFunctions.find((m) => fnStr(m).match(new RegExp(`\\([\\w$]+\\.${type}(?!_)`)));
      if (!func) continue;

      const camelCaseType = toCamelCase(type);
      Navify.URI[`${camelCaseType}URI`] = func;
    }
    ...
    for (const type of Object.keys(Navify.URI.Type)) {
      ...
      Navify.URI[`is${camelCaseType}`] =
        func ??
        ((uri) => {
          let uriObj;
          try {
            uriObj = Navify.URI.from?.(uri) ?? Navify.URI.fromString?.(uri);
          } catch {
            return false;
          }
          if (!uriObj) return false;
          return uriObj.type === Navify.URI.Type[type];
        });
    }

    Navify.URI.isPlaylistV1OrV2 = (uri) => Navify.URI.isPlaylist(uri) || Navify.URI.isPlaylistV2(uri);

    const base62 = findBase62Module(inventory.moduleCandidates);

    Navify.URI.idToHex ??= findIDToHex(allFunctions) ?? (base62 ? createIDToHex(base62) : undefined);
    Navify.URI.hexToId ??= findHexToID(allFunctions, Navify.URI.idToHex) ?? (base62 ? createHexToID(base62) : undefined);

    // isSameIdentity
    Navify.URI.isSameIdentity ??= URIModules.find((m) => fnStr(m).match(/[\w$]+\.id===[\w$]+\.id/));
  })();
}
