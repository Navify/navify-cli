...
Navify.getAudioData = async (uri) => {
  const providedURI = uri || Navify.Player.data.item.uri;
  const uriObj = Navify.URI.from?.(providedURI) ?? Navify.URI.fromString?.(providedURI);
  if (!uriObj || (uriObj.Type || uriObj.type) !== Navify.URI.Type.TRACK) {
    throw "URI is invalid.";
  }

  return await Navify.CosmosAsync.get(
    `https://spclient.wg.spotify.com/audio-attributes/v1/audio-analysis/${uriObj.getBase62Id?.() ?? uriObj.id}?format=json`,
  );
};

Navify.colorExtractor = async (uri) => {
  const body = await Navify.CosmosAsync.get(`https://spclient.wg.spotify.com/colorextractor/v1/extract-presets?uri=${uri}&format=json`);
...
Navify._getStyledClassName = (args, component) => {
  const includedKeys = [
    "role",
    "variant",
    "semanticColor",
...
void (async function waitMouseTrap() {
  await waitFor(() => Navify.Mousetrap, 10);
  const KEYS = {
    BACKSPACE: "backspace",
    TAB: "tab",
    ENTER: "enter",
  ...
  function formatKeys(keys) {
    ...
    if (typeof keys === "object") {
      if (!keys.key || !Object.values(KEYS).includes(keys.key)) {
        throw `Navify.Keyboard.registerShortcut: Invalid key ${keys.key}`;
      }
      ...
      throw `Navify.Keyboard.registerShortcut: Invalid keys ${keys}`;
    }
    return keystroke;
  }
  ...
  Navify.Keyboard = {
    ...
    registerShortcut: (keys, callback) => {
      Navify.Mousetrap.bind(formatKeys(keys), callback);
    },
    _deregisterShortcut: (keys) => {
      Navify.Mousetrap.unbind(formatKeys(keys));
    },
    changeShortcut: (keys, newKeys) => {
      if (!keys || !newKeys) throw "Navify.Keyboard.changeShortcut: Invalid keys";

      const callback = Object.keys(Navify.Mousetrap.trigger()._directMap).find((key) => key.startsWith(formatKeys(keys)));
      if (!callback) throw "Navify.Keyboard.changeShortcut: Shortcut not found";

      Navify.Keyboard.registerShortcut(newKeys, Navify.Mousetrap.trigger()._directMap[callback]);
      Navify.Keyboard._deregisterShortcut(keys);
    },
  };
  Navify.Keyboard.registerIsolatedShortcut = Navify.Keyboard.registerShortcut;
  Navify.Keyboard.registerImportantShortcut = Navify.Keyboard.registerShortcut;
  Navify.Keyboard.deregisterImportantShortcut = Navify.Keyboard._deregisterShortcut;
})();
