...
type Flag struct {
	...
	RemoveRTL bool
	// ExposeAPIs leaks Spotify's API, functions, objects to Navify global object.
	ExposeAPIs bool
	SpotifyVer string
}
...
func readRemoteCssMap(tag string, cssTranslationMap *map[string]string) error {
	var cssMapURL string = "https://raw.githubusercontent.com/Navify/navify-cli/" + tag + "/css-map.json"
...
func Start(version string, spotifyBasePath string, extractedAppsPath string, flags Flag) {
	...
	case "linux":
		candidate := filepath.Join(spotifyBasePath, "spotify")
		if _, err := os.Stat(candidate); err == nil {
			spotifyBinaryPath = candidate
		}
	}
	...
	for _, path := range filesToPatch {
				...
				if flags.ExposeAPIs {
					...
					if spotifyMajor >= 1 && spotifyMinor >= 2 && spotifyPatch < 78 {
						utils.ReplaceOnce(&content, `\(\({[^}]*,\s*imageSrc`, func(submatches ...string) string {
							return fmt.Sprintf("Navify.Snackbar.enqueueImageSnackbar=%s", submatches[0])
						})
					}
		...
		case ".html":
			utils.ModifyFile(path, func(content string) string {
				...
				if flags.ExposeAPIs {
					tags += "<script src='helper/navifyWrapper.js'></script>\n"
					tags += "<!-- navify helpers -->\n"
				}
	...
	graphQLPatches := []Patch{
		{
			...
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf("%sNavify.GraphQL.Definitions[%s]=%s", submatches[1], submatches[3], submatches[2])
			},
		},
		{
			...
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf(`=Navify.GraphQL.Definitions["%s"]%s`, submatches[2], submatches[1])
			},
		},
	}
...
func exposeAPIs_main(input string) string {
	...
	if len(inputContextMenu) > 0 {
		...
		utils.Replace(&input, `\(0,([\w_$]+)\.jsx\)\((?:[\w_$]+\.[\w_$]+,\{value:"contextmenu"[^}]+\}\)\}\)|"[\w-]+",\{[^}]+:"context-menu"[^}]+\}\))`, func(submatches ...string) string {
			return fmt.Sprintf("(0,%s.jsx)((Navify.ContextMenuV2._context||(Navify.ContextMenuV2._context=%s.createContext(null))).Provider,{value:{props:%s?.props,trigger:%s,target:%s},children:%s})", submatches[1], react, menu, trigger, target, submatches[0])
		})
	}
	...
	xpuiPatches := []Patch{
		{
			...
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf(`;globalThis.Navify.showNotification=(message,isError=false,msTimeout)=>%s({message,feedbackType:isError?"ERROR":"NOTICE",msTimeout});const %s=%s`, submatches[1], submatches[1], submatches[2])
			},
		},
		{
		...
		},
		{
			...
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf("%sNavify._platform=%s", submatches[1], submatches[2])
			},
		},
		{
			Name:  "Redux store",
			...
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf("%sNavify.Platform.ReduxStore=%s", submatches[1], submatches[2])
			},
		},
		{
			...
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf("%sNavify.ReactComponent.PlatformProvider=%s", submatches[1], submatches[2])
			},
		},
		{
		...
		},
		{
			...
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf("%sNavify.Snackbar.enqueueCustomSnackbar=%s;", submatches[0], submatches[1])
			},
		},
		{
			...
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf("=Navify.Snackbar.enqueueCustomSnackbar%s;", submatches[0])
			},
		},
		{
			...
			Regex: `(=)(\(\({[^}]*,\s*imageSrc)`,
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf("%sNavify.Snackbar.enqueueImageSnackbar=%s", submatches[1], submatches[2])
			},
		},
		{
			...
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf("%sNavify.ReactComponent.Navigation=%s", submatches[1], submatches[2])
			},
			Once: true,
		},
		{
			...
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf("%s[Navify.ContextMenuV2.renderItems(),%s].flat()", submatches[1], submatches[2])
			},
		},
	}
...
func exposeAPIs_vendor(input string) string {
	...
	utils.Replace(
		...
		func(submatches ...string) string {
			return fmt.Sprintf(`,(globalThis.Navify.URI=%s)%s`, submatches[1], submatches[0])
		})
	vendorPatches := []Patch{
		{
			Name:  "Navify.URI",
		...
		},
		{
			...
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf("%s=Navify._getStyledClassName(arguments,this)", submatches[1])
			},
		},
		{
			Name:  "Tippy.js",
			...
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf("Navify.Tippy=%s;%s", submatches[1], submatches[0])
			},
		},
		{
			...
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf("%s=Navify.ReactFlipToolkit.spring=%s", submatches[1], submatches[2])
			},
		},
		{
			...
			Replacement: func(submatches ...string) string {
				return fmt.Sprintf("Navify.Snackbar=%s", submatches[0])
			},
		},
	}

	// URI after 1.2.4
	if !strings.Contains(input, "Navify.URI") {
		...
		if len(URIObj) != 0 {
			...
			input = strings.Replace(
				input,
				URI,
				fmt.Sprintf("%s;Navify.URI=%s;", URI, URIObj[1]),
				1)
		}
	}
...
func validateReleaseBuild(spotifyBinaryPath string) error {
	...
	if buildType != "Release" {
		return fmt.Errorf("detected %s Spotify build! navify works only on Release builds. Please install latest Release version of Spotify", buildType)
	}
...
func FetchLatestTagMatchingVersion(version string) (string, error) {
	...
	res, err := http.Get("https://api.github.com/repos/Navify/navify-cli/releases")
	if err != nil {
		return "", err
	}
