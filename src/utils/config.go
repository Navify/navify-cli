...
func linuxApp() string {
	...
	potentialList := []string{
		...
		"/var/lib/flatpak/app/com.spotify.Client/aarch64/stable/active/files/extra/share/spotify/",
...
func linuxPrefs() string {
	...
	potentialList := []string{
		filepath.Join(dotConfig, "spotify", "prefs"),
		filepath.Join(os.Getenv("HOME"), ".var", "app", "com.spotify.Client", "config", "spotify", "prefs"),
		filepath.Join(os.Getenv("HOME"), "snap", "spotify", "common", ".config", "spotify", "prefs"),
	}

	for _, pref := range potentialList {
