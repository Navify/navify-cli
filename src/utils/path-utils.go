...
func MigrateConfigFolder() {
	if runtime.GOOS == "windows" {
		source := filepath.Join(os.Getenv("USERPROFILE"), ".navify")
		if _, err := os.Stat(source); err == nil {
			spinner, _ := Spinner.Start("Migrating config folder")
			destination := GetNavifyFolder()
...
func MigrateFolders() {
	backupPath := filepath.Join(GetNavifyFolder(), "Backup")
	extractedPath := filepath.Join(GetNavifyFolder(), "Extracted")
...
func GetNavifyFolder() string {
	result, isAvailable := os.LookupEnv("NAVIFY_CONFIG")
...
func GetStateFolder(name string) string {
	result, isAvailable := os.LookupEnv("NAVIFY_STATE")
	defer func() { CheckExistAndCreate(result) }()
...
var userAppsFolder = GetSubFolder(GetNavifyFolder(), "CustomApps")
var userExtensionsFolder = GetSubFolder(GetNavifyFolder(), "Extensions")
