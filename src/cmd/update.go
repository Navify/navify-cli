...
func Update(currentVersion string) bool {
	...
	if currentVersion == tagName {
		utils.PrintSuccess("Navify is up-to-date.")
		return false
	}

	utils.PrintInfo("Latest release: " + tagName)
	var assetURL string = "https://github.com/Navify/navify-cli/releases/download/v" + tagName + "/navify-" + tagName + "-" + runtime.GOOS + "-"
	var location string = os.TempDir() + "/navify-" + tagName
	...
	spinner, _ := utils.Spinner.Start("Downloading Navify")

	out, err := os.Create(location)
	if err != nil {
		spinner.Fail("Failed to download Navify")
		utils.Fatal(err)
	}
	defer out.Close()
	...
	spinner.Success("Downloaded Navify")
	...
	utils.CheckExistAndDelete(exeOld)
	utils.PrintSuccess("Successfully updated Navify to v" + tagName)
	return true
}

func permissionError(err error) {
	utils.PrintInfo("If fatal error is \"Permission denied\", please check read/write permission of navify executable directory.")
	utils.PrintInfo("However, if you used a package manager to install navify, please upgrade by using the same package manager.")
	utils.Fatal(err)
}
