...
// tries to auto-detect them and stops navify when any one
// of them is invalid.
func InitPaths() {
	...
	if isAppX {
		appDestPath = filepath.Join(navifyFolder, "AppX")
	} else {
		appDestPath = appPath
	}
...
func GetConfigPath() string {
	return filepath.Join(navifyFolder, "config-xpui.ini")
}
...
func CheckUpdate(version string) {
	if !settingSection.Key("check_navify_update").MustBool() || version == "Dev" {
		return
	}
	...
	if latestTag != version {
		...
		utils.PrintInfo(`Run "navify update" or use a package manager to update navify`)
	}
}
