...
import (
	...
	"github.com/navify/cli/src/apply"
	backupstatus "github.com/navify/cli/src/status/backup"
	spotifystatus "github.com/navify/cli/src/status/spotify"
	"github.com/navify/cli/src/utils"
)

// Apply .
func Apply(navifyVersion string) {
	utils.MigrateConfigFolder()

	backupNavifyVersion := backupSection.Key("with").MustString("")
	if navifyVersion != backupNavifyVersion {
		utils.PrintInfo(`Preprocessed Spotify data is outdated. Please run "navify restore backup apply" to receive new features and bug fixes`)
		os.Exit(1)
	}
	...
	if preprocSection.Key("expose_apis").MustBool(false) {
		utils.CopyFile(
			filepath.Join(utils.GetJsHelperDir(), "navifyWrapper.js"),
...
func refreshThemeCSS() {
	...
	var configJson navifyConfigJson
	...
	if err != nil {
		...
		if err := os.WriteFile(
			filepath.Join(appDestPath, "xpui", "navify-config.json"),
...
func CheckStates() {
	...
	if backStat.IsEmpty() {
		if spotStat.IsBackupable() {
			utils.PrintError(`You haven't backed up. Run "navify backup apply"`)
		} else {
			utils.PrintError(`You haven't backed up and Spotify cannot be backed up at this state. Please re-install Spotify then run "navify backup apply"`)
		}
		os.Exit(1)
		...
		if spotStat.IsMixed() {
			...
			utils.PrintInfo(`Please run "navify backup apply"`)
		} else if spotStat.IsStock() {
			utils.PrintInfo(`Spotify client is in stock state`)
			utils.PrintInfo(`Please run "navify backup apply"`)
		} else {
			utils.PrintInfo(`Spotify cannot be backed up at this state. Please re-install Spotify then run "navify backup apply"`)
		}
