...
func AdditionalOptions(appsFolderPath string, flags Flag) {
	...
	if len(flags.CustomApp) > 0 {
		if customAppTarget := findCustomAppTarget(appsFolderPath); customAppTarget != "" {
			...
			utils.PrintError("Spotify version mismatch with Navify. Please report it on our github repository.")
			utils.PrintInfo("Navify might have been updated for this version already. Please run `navify update` to check for a new version.")
...
func htmlMod(htmlPath string, flags Flag) {
	...
	if flags.NavifyVer != "" {
		...
		fmt.Fprintf(&helperHTML, `<script>
			Navify.Config={};
			Navify.Config["version"]="%s";
			Navify.Config["current_theme"]="%s";
			Navify.Config["color_scheme"]="%s";
			Navify.Config["extensions"] = [%s];
			Navify.Config["custom_apps"] = [%s];
			Navify.Config["check_navify_update"]=%v;
		</script>
		`, flags.NavifyVer, flags.CurrentTheme, flags.ColorScheme, extList.String(), customAppList.String(), flags.CheckNavifyUpdate)
	}
...
func insertNavLink(str string, appNameArray string) string {
	...
	if libraryXItemMatch != "" {
		str = strings.Replace(
			str,
			libraryXItemMatch,
			fmt.Sprintf("%s,Navify._renderNavLinks([%s], false)", libraryXItemMatch, appNameArray),
			1)
	}
	...
	utils.ReplaceOnceWithPriority(&str,
		...
		func(index int, submatches ...string) string {
			switch index {
			case 0, 1:
				return fmt.Sprintf("%s,Navify._renderNavLinks([%s], true)]", submatches[1], appNameArray)
			case 2:
				return fmt.Sprintf("%s[%s,Navify._renderNavLinks([%s], true)].flat()%s", submatches[1], submatches[2], appNameArray, submatches[3])
			case 3:
				return fmt.Sprintf("%s[%s%s,Navify._renderNavLinks([%s], true)].flat()", submatches[1], submatches[2], submatches[3], appNameArray)
			}
			return ""
		},
	)
...
func insertHomeConfig(jsPath string, flags Flag) {
	...
	utils.ModifyFile(jsPath, func(content string) string {
		utils.ReplaceOnce(
			...
			func(submatches ...string) string {
				return fmt.Sprintf("%sNavifyHomeConfig.arrange(%s)%s", submatches[1], submatches[2], submatches[3])
			})
...
func insertExpFeatures(jsPath string, flags Flag) {
	...
	utils.ModifyFile(jsPath, func(content string) string {
		utils.ReplaceOnce(
			...
			func(submatches ...string) string {
				return fmt.Sprintf("%s%s=Navify.expFeatureOverride(%s);%s", submatches[1], submatches[2], submatches[2], submatches[3])
			})
		...
		// 	func(submatches ...string) string {
		// 		return fmt.Sprintf("%s=Navify.createInternalMap%s", submatches[1], submatches[2])
		// 	})
		...
		utils.ReplaceOnce(
			...
			func(submatches ...string) string {
				return fmt.Sprintf("%sNavify.createInternalMap=%s;%sNavify.RemoteConfigResolver=%s", submatches[1], submatches[2], submatches[3], submatches[4])
			})
		...
		utils.ReplaceOnce(
			...
			func(submatches ...string) string {
				return fmt.Sprintf(`%s%s("details",{children: [
					%s("summary",{children: "Navify v" + Navify.Config.version}),
					%s("li",{children: "Theme: " + Navify.Config.current_theme + (Navify.Config.color_scheme && " / ") + Navify.Config.color_scheme}),
					%s("li",{children: "Extensions: " + Navify.Config.extensions.join(", ")}),
					%s("li",{children: "Custom apps: " + Navify.Config.custom_apps.join(", ")}),
