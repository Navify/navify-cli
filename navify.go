...
func init() {
	...
	if isAdmin.Check(bypassAdminCheck) {
		utils.PrintError("Navify should NOT be run with administrator or root privileges")
...
func main() {
		...
		path, err := (func() (string, error) {
			if styleFocus {
				...
				if len(commands) == 0 && len(flags) == 0 {
					...
					return utils.GetNavifyFolder(), nil
				}
	...
	utils.PrintBold("navify v" + version)
	if slices.Contains(commands, "upgrade") || slices.Contains(commands, "update") {
		...
		spotStat := spotifystatus.Get(spotifyPath)
		if spotStat.IsBackupable() {
			utils.PrintNote("navify is up-to-date! If you ran this because navify disappeared after Spotify updated, we'll attempt to fix it for you right now.")
...
Run "navify -h" for a list of valid commands.`))
		}
	}
...
func help() {
	...
	log.Println(utils.Bold("USAGE") + "\n" +
		"navify [-q] [-e] [-a] \x1B[4mcommand\033[0m...\n" +
		"navify {-c | --config} | {-v | --version} | {-h | --help}\n\n" +
...
path                Print path of Spotify's executable, userdata, and more.
                    ...
                    navify path userdata

                    3. Print all paths:
                    navify path all

                    4. Toggle focus with flags:
                    navify path <flag> <option>
...
config              1. Print all config fields and values:
                    ...
                    navify config custom_apps

                    3. Change value of one or multiple config fields.
                    navify config <field> <value> [<field> <value> ...]
                    ...
                    navify config disable_sentry 1
                    - Add extension "myFakeExt.js" to current extensions list:
                    navify config extensions myFakeExt.js
                    - Remove extension "wrongname.js" from extensions list:
                    navify config extensions wrongname.js-
                    - Disable "inject_css" and enable "song_page"
                    navify config inject_css 0 song_page 1
...
color               1. Print all color fields and values.
                    ...
                    navify color <field> <value> [<field> <value> ...]

                    <value> can be in hex or decimal (rrr,ggg,bbb) format.

                    Example usage:
                    - Change main to ff0000
                    navify color main ff0000
                    - Change sidebar to 00ff00 and button to 0000ff
                    navify color sidebar 00ff00 button 0000ff

config-dir          Show config directory in file viewer

upgrade|update      Update navify to the latest version if an update is available
...
spotify_launch_flags <string>
    ...
    List of valid flags: https://navify.app/docs/development/spotify-cli-flags

always_enable_devtools <0 | 1>
    Whether Chrome DevTools is enabled when launching/restarting Spotify.

check_navify_update <0 | 1>
    Whether to always check for updates when running Navify.
...
expose_apis <0 | 1>
    Leaks some Spotify's API, functions, objects to Navify global object that
